...
class ResSFLTrainer:
    ...
    def __init__(self, client_model, server_model, train_loader, test_loader):
        ...
        self.ae_optimizer = optim.Adam(self.local_ae.parameters(), lr=self.AE_LR)

        milestones = [15, 30, 40]
        self.server_scheduler = optim.lr_scheduler.MultiStepLR(self.server_optimizer, milestones=milestones, gamma=0.2)
        self.client_scheduler = optim.lr_scheduler.MultiStepLR(self.client_optimizer, milestones=milestones, gamma=0.2)
        self.ae_scheduler = optim.lr_scheduler.MultiStepLR(self.ae_optimizer, milestones=milestones, gamma=0.2)
    ...
    def pretrain_ae(self, pre_epochs=30, ae_batch_size=32, collect_batches=80):
        ...
        for epoch in range(pre_epochs):
            ...
            if val_mse < best_val_mse:
                ...
                best_state   = {k: v.clone() for k, v in self.local_ae.state_dict().items()}
    ...
    def _train_one_epoch(self, epoch):
        ...
        progress_bar = tqdm(self.train_loader, desc=f"  ResSFL Epoch [{epoch+1}/{Config.EPOCHS}]", leave=False)
        ...
        for batch_idx, (inputs, labels) in enumerate(progress_bar):
            ...
            total_loss, ce_loss, ssim_val = self._train_target_step(inputs, labels)
